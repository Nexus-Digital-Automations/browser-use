...
class DefaultActionWatchdog(BaseWatchdog):
    ...
    async def on_ClickElementEvent(self, event: ClickElementEvent) -> dict | None:
        ...
        try:
            ...
            if (
                not self.browser_session.agent_focus
                or not self.browser_session.agent_focus.target_id
            ):
            ...
            click_metadata = None
            click_metadata = await self._click_element_node_impl(
                element_node, while_holding_ctrl=event.while_holding_ctrl
            )
            ...
            self.browser_session.agent_focus = (
            ...
            new_target_ids = {t["targetId"] for t in after_target_ids} - {
                t["targetId"] for t in initial_target_ids
            }
            ...
            if new_target_ids:
                ...
                if not event.while_holding_ctrl:
                    ...
                    switch_event = await self.event_bus.dispatch(
        ...
        try:
            ...
            if not element_node.element_index or element_node.element_index == 0:
                ...
                try:
                    ...
                    self.logger.info(
                        f'⌨️ Typed "{event.text}" into element with index {index_for_logging}'
                    )
                    self.logger.debug(f"Element xpath: {element_node.xpath}")
                    ...
                    self.logger.warning(
                        f"Failed to type to element {index_for_logging}: {e}. Falling back to page typing."
                    )
                    try:
                        await asyncio.wait_for(
                            self._click_element_node_impl(
                                element_node, while_holding_ctrl=False
                            ),
                            timeout=3.0,
                        )
                    except Exception as e:
    ...
    async def on_ScrollEvent(self, event: ScrollEvent) -> None:
        ...
        try:
            ...
            if event.node is not None:
                ...
                is_iframe = (
                    element_node.tag_name and element_node.tag_name.upper() == "IFRAME"
                )
    ...
    async def _click_element_node_impl(
        self, element_node, while_holding_ctrl: bool = False
    ) -> dict | None:
        """
        ...
        try:
            ...
            layout_metrics = await cdp_session.cdp_client.send.Page.getLayoutMetrics(
            ...
            try:
                content_quads_result = (
                    await cdp_session.cdp_client.send.DOM.getContentQuads(
                ...
                if "quads" in content_quads_result and content_quads_result["quads"]:
                    ...
                    self.logger.debug(
                        f"Got {len(quads)} quads from DOM.getContentQuads"
                    )
            except Exception as e:
            ...
            if not quads:
                self.logger.warning(
                    "⚠️ Could not get element geometry from any method, falling back to JavaScript click"
                )
                try:
                    result = await cdp_session.cdp_client.send.DOM.resolveNode(
                        params={"backendNodeId": backend_node_id},
                    ...
                    assert (
                        "object" in result and "objectId" in result["object"]
            ...
            try:
                self.logger.debug(
                    f"👆 Dragging mouse over element before clicking x: {center_x}px y: {center_y}px ..."
                )
                ...
                # Mouse down
                self.logger.debug(
                    f"👆🏾 Clicking x: {center_x}px y: {center_y}px with modifiers: {modifiers} ..."
                )
                try:
                    await asyncio.wait_for(
                    ...
                    self.logger.debug(
                        "⏱️ Mouse down timed out (likely due to dialog), continuing..."
                    )
                ...
                try:
                    ...
                    self.logger.debug(
                        "⏱️ Mouse up timed out (possibly due to lag or dialog popup), continuing..."
                    )
    ...
    async def _clear_text_field(self, object_id: str, cdp_session) -> bool:
        ...
        try:
            ...
            if not current_value:
                ...
                self.logger.debug(
                    f'⚠️ JavaScript clear partially failed, field still contains: "{current_value}"'
                )
        ...
        try:
            ...
            self.logger.debug(
                f"🧹 Last resort: Clearing using {modifier_name}+A + Backspace"
            )
    ...
    async def _focus_element_simple(
        ...
        try:
            ...
            self.logger.debug(
                f"❌ CDP DOM.focus threw exception: {type(e).__name__}: {e}"
            )

        # Strategy 2: Try click to focus if CDP failed
        if (
            input_coordinates
            and "input_x" in input_coordinates
            and "input_y" in input_coordinates
        ):
            try:
                ...
                self.logger.debug(
                    f"🎯 Attempting click-to-focus at ({click_x:.1f}, {click_y:.1f})"
                )
    ...
    async def _input_text_element_node_impl(
        ...
        try:
            ...
            if element_node.absolute_position:
                center_x = (
                    element_node.absolute_position.x
                    + element_node.absolute_position.width / 2
                )
                center_y = (
                    element_node.absolute_position.y
                    + element_node.absolute_position.height / 2
                )
                input_coordinates = {"input_x": center_x, "input_y": center_y}
                self.logger.debug(
                    f"Using absolute_position coordinates: x={center_x:.1f}, y={center_y:.1f}"
                )
            else:
                input_coordinates = None
            ...
            if clear_existing and focused_successfully:
                cleared_successfully = await self._clear_text_field(
                    object_id=object_id, cdp_session=cdp_session
                )
                if not cleared_successfully:
                    self.logger.warning(
                        "⚠️ Text field clearing failed, typing may append to existing text"
                    )
    ...
    async def _scroll_with_cdp_gesture(self, pixels: int) -> bool:
        ...
        try:
            ...
            layout_metrics = await cdp_client.send.Page.getLayoutMetrics(
                session_id=session_id
            )
            viewport_width = layout_metrics["layoutViewport"]["clientWidth"]
            viewport_height = layout_metrics["layoutViewport"]["clientHeight"]
    ...
    async def _scroll_element_container(self, element_node, pixels: int) -> bool:
        ...
        try:
            ...
            center_x = (
                content_quad[0] + content_quad[2] + content_quad[4] + content_quad[6]
            ) / 4
            center_y = (
                content_quad[1] + content_quad[3] + content_quad[5] + content_quad[7]
            ) / 4
    ...
    async def _get_session_id_for_element(
        self, element_node: EnhancedDOMTreeNode
    ) -> str | None:
        ...
        if element_node.frame_id:
            ...
            try:
                ...
                for target in targets["targetInfos"]:
                    if target["type"] == "iframe" and element_node.frame_id in str(
                        target.get("targetId", "")
                    ):
                ...
                self.logger.debug(
                    f"Frame {element_node.frame_id} not found in targets, using main session"
                )
            except Exception as e:
                self.logger.debug(
                    f"Error getting frame session: {e}, using main session"
                )
    ...
    async def on_GoBackEvent(self, event: GoBackEvent) -> None:
        ...
        try:
            ...
            history = await cdp_session.cdp_client.send.Page.getNavigationHistory(
                session_id=cdp_session.session_id
            )
            ...
            self.logger.info(
                f'🔙 Navigated back to {entries[current_index - 1]["url"]}'
            )
        except Exception as _e:
            raise
    ...
    async def on_GoForwardEvent(self, event: GoForwardEvent) -> None:
        ...
        try:
            ...
            self.logger.info(
                f'🔜 Navigated forward to {entries[current_index + 1]["url"]}'
            )
        except Exception as _e:
            raise
    ...
    async def on_WaitEvent(self, event: WaitEvent) -> None:
        ...
        try:
            ...
            if actual_seconds != event.seconds:
                self.logger.info(
                    f"🕒 Waiting for {actual_seconds} seconds (capped from {event.seconds}s)"
                )
            else:
    ...
    async def on_UploadFileEvent(self, event: UploadFileEvent) -> None:
        ...
        try:
            ...
            if not self.browser_session.is_file_input(element_node):
                msg = (
                    f"Upload failed - element {index_for_logging} is not a file input."
                )
            ...
            self.logger.info(
                f"📎 Uploaded file {event.file_path} to element {index_for_logging}"
            )
        except Exception as _e:
            raise
    ...
    async def on_ScrollToTextEvent(self, event: ScrollToTextEvent) -> None:
        ...
        doc = await cdp_client.send.DOM.getDocument(
            params={"depth": -1}, session_id=session_id
        )
        _root_node_id = doc["root"]["nodeId"]
        ...
        for query in search_queries:
            try:
                ...
                await cdp_client.send.DOM.discardSearchResults(
                    params={"searchId": search_id}, session_id=session_id
                )
            except Exception as e:
        ...
        if js_result.get("result", {}).get("value"):
            ...
            raise BrowserError(
                f'Text not found: "{event.text}"', details={"text": event.text}
            )
    ...
    async def on_GetDropdownOptionsEvent(
        self, event: GetDropdownOptionsEvent
    ) -> dict[str, str]:
        ...
        try:
            ...
            for opt in dropdown_data["options"]:
                ...
                formatted_options.append(
                    f'{opt["index"]}: text={encoded_text}, value={json.dumps(opt["value"])}{status}'
                )
            ...
            if source_info == "target":
                ...
                msg = (
                    f"Found {dropdown_type} dropdown in {source_info} ({element_info}):\n"
            ...
            if source_info == "target":
                self.logger.info(
                    f'📋 Found {len(dropdown_data["options"])} dropdown options for index {index_for_logging}'
                )
            else:
                self.logger.info(
            ...
            return {
                ...
                ),  # Convert list to JSON string for dict[str, str] type
        ...
        try:
            ...
            try:
                ...
                if selection_result.get("success"):
                    msg = selection_result.get(
                        "message", f"Selected option: {target_text}"
                    )
                    ...
                    error_msg = selection_result.get(
                        "error", f"Failed to select option: {target_text}"
                    )
                    available_options = selection_result.get("availableOptions", [])
                    self.logger.error(f"❌ {error_msg}")
                    self.logger.debug(
                        f"Available options from JavaScript: {available_options}"
                    )
                    ...
                    if available_options:
                        ...
                        if short_term_options:
                            short_term_memory = (
                                f"Available dropdown options at index {index_for_logging} are:\n"
