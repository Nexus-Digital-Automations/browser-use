...
class TestClickElementEvent:
    ...
    async def test_error_handling(self, tools, browser_session):
        ...
        invalid_action = {
            "click_element_by_index": ClickElementAction(index=999)
        ...
        result: ActionResult = await tools.act(
            ClickActionModel(**invalid_action), browser_session
        )

        assert result.error is not None

    async def test_click_element_by_index(
        self, tools, browser_session, base_url, http_server
    ):
        ...
        goto_action = {
            "go_to_url": GoToUrlAction(url=f"{base_url}/clickable", new_tab=False)
        }
        ...
        for idx, element in selector_map.items():
            ...
            if element.tag_name.lower() == "div" and "clickable" in str(
        ...
        assert (
            button_text is not None and expected_button_text in button_text
        ...
        result = await tools.act(
            ClickElementActionModel(
                click_element_by_index=ClickElementAction(index=button_index)
            ),
            browser_session,
        )
        ...
        assert isinstance(
            result, ActionResult
        ), "Result should be an ActionResult instance"
        ...
        assert (
            result_text == expected_result_text
        ), f"Expected result text '{expected_result_text}', got '{result_text}'"

    async def test_click_element_new_tab(
        self, tools, browser_session, base_url, http_server
    ):
        ...
        goto_action = {
            "go_to_url": GoToUrlAction(url=f"{base_url}/newTab", new_tab=False)
        }
        ...
        click_action = {
            "click_element_by_index": ClickElementAction(
                index=link_index, while_holding_ctrl=True
            )
        }
        ...
        assert result_text is not None
        assert (
            f"Clicked element with index {link_index}" in result_text
        ), f"Expected click confirmation in result content, got: {result_text}"
        ...
        assert (
            final_tab_count == initial_tab_count + 1
        ), f"Expected {initial_tab_count + 1} tabs, got {final_tab_count}"

        # Verify we're still on the original tab (not switched) - matches browser Cmd/Ctrl+click behavior
        current_url = await browser_session.get_current_page_url()
        assert (
            f"{base_url}/newTab" in current_url
        ), f"Should still be on original tab, but got {current_url}"
        ...
        assert (
            f"{base_url}/page1" in new_tab.url or f"{base_url}/newTab" in new_tab.url
        ), f"New tab should have page1 or newTab URL, but got {new_tab.url}"

    @pytest.mark.skip(
        reason="Tab count assertion failures - tab management logic changed"
    )
    async def test_click_element_normal_vs_new_tab(
        self, tools, browser_session, base_url, http_server
    ):
        ...
        goto_action = {
            "go_to_url": GoToUrlAction(url=f"{base_url}/comparison", new_tab=False)
        }
        ...
        click_action_normal = {
            "click_element_by_index": ClickElementAction(
                index=link_indices[0], while_holding_ctrl=False
            )
        }
    ...
    async def test_inline_element_mostly_offscreen(
        self, tools, browser_session, base_url, http_server
    ):
        ...
        goto_action = {
            "go_to_url": GoToUrlAction(
                url=f"{base_url}/inline_offscreen", new_tab=False
            )
        }
        ...
        result = await tools.act(
            ClickActionModel(
                click_element_by_index=ClickElementAction(index=inline_index)
            ),
            browser_session,
        )
    ...
    async def test_block_inside_inline_multiline(
        self, tools, browser_session, base_url, http_server
    ):
        ...
        # Navigate to the page
        goto_action = {
            "go_to_url": GoToUrlAction(url=f"{base_url}/block_in_inline", new_tab=False)
        }
        ...
        result = await tools.act(
            ClickActionModel(
                click_element_by_index=ClickElementAction(index=block_index)
            ),
            browser_session,
        )
    ...
    async def test_element_covered_by_overlay(
        self, tools, browser_session, base_url, http_server
    ):
        ...
        # Navigate to the page
        goto_action = {
            "go_to_url": GoToUrlAction(url=f"{base_url}/covered_element", new_tab=False)
        }
        ...
        result = await tools.act(
            ClickActionModel(
                click_element_by_index=ClickElementAction(index=target_index)
            ),
            browser_session,
        )
    ...
    async def test_file_input_click_prevention(
        self, tools, browser_session, base_url, http_server
    ):
        ...
        # Navigate to the page
        goto_action = {
            "go_to_url": GoToUrlAction(url=f"{base_url}/file_input", new_tab=False)
        }
        ...
        result = await tools.act(
            ClickActionModel(
                click_element_by_index=ClickElementAction(index=file_input_index)
            ),
            browser_session,
        )
    ...
    async def test_select_dropdown_click_prevention(
        ...
        # Navigate to the page
        goto_action = {
            "go_to_url": GoToUrlAction(url=f"{base_url}/select_dropdown", new_tab=False)
        }
        ...
        result = await tools.act(
            ClickActionModel(
                click_element_by_index=ClickElementAction(index=select_index)
            ),
            browser_session,
        )
        ...
        ), "Should not have error - should provide dropdown options automatically"
        assert (
            result.extracted_content is not None
        ), "Should have dropdown options content"
        assert (
            "dropdown" in result.extracted_content.lower()
        ), f"Should contain dropdown options, got: {result.extracted_content}"

    @pytest.mark.skip(
        reason="Dialog system validation bug - DialogOpenedEvent.frame_id expects string but gets None"
    )
    async def test_click_triggers_alert_popup(
        self, browser_session, base_url, http_server
    ):
    ...
    async def test_click_triggers_confirm_popup(
        self, browser_session, base_url, http_server
    ):
    ...
    async def test_page_usable_after_popup_confirm(
        self, browser_session, base_url, http_server
    ):
        ...
        current_url = await browser_session.get_current_page_url()
        assert (
            f"{base_url}/page1" in current_url
    ...
    async def test_click_triggers_onbeforeunload_popup(
        self, browser_session, base_url, http_server
    ):
    ...
    async def test_file_upload_click_and_verify(
        self, tools, browser_session, base_url, http_server
    ):
        ...
        try:
            ...
            goto_action = {
                "go_to_url": GoToUrlAction(url=f"{base_url}/fileupload", new_tab=False)
            }
            ...
            label_index = None
            for idx, element in selector_map.items():
                if element.tag_name.lower() == "label" and "upload-label" in str(
                    element.attributes.get("class", "")
                ):
                    label_index = idx
                    break
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                result = await tools.act(
                    UploadFileActionModel(
                        upload_file_to_element=UploadFileAction(
                            index=label_index, path=temp_file_path
                        )
                    ),
                    browser_session,
                    available_file_paths=[
                        temp_file_path
                    ],  # Pass the file path as available
                ...
                assert (
                    file_info.get("hasFile") is True
                ), "File was not properly selected in the input element"
                assert file_info.get("fileName", "").endswith(
                    ".txt"
                ), f'Expected .txt file, got: {file_info.get("fileName")}'
                assert (
                    file_info.get("fileSize", 0) > 0
                ...
                assert (
                    ui_info.get("hasFileInfo") is True
                ), "UI was not updated with file information"
                assert ".txt" in ui_info.get(
                    "fileNameText", ""
                ), f'File name not shown in UI: {ui_info.get("fileNameText")}'
                assert "bytes" in ui_info.get(
                    "fileSizeText", ""
                ), f'File size not shown in UI: {ui_info.get("fileSizeText")}'

        finally:
            # Clean up the temporary file
            Path(temp_file_path).unlink(missing_ok=True)

    async def test_file_upload_path_validation(
        self, tools, browser_session, base_url, http_server
    ):
        ...
        try:
            ...
            # Navigate to the test page
            goto_action = {
                "go_to_url": GoToUrlAction(url=f"{base_url}/upload-test", new_tab=False)
            }
            from browser_use.agent.views import ActionModel
            ...
            upload_action = UploadActionModel(
                upload_file_to_element=UploadFileAction(index=1, path=test_file_path)
            )
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    ), "Upload should have failed for file not in available_file_paths"
                    assert (
                        "not available" in result.error
                    ), f"Error message should mention file not available: {result.error}"
                except BrowserError as e:
                    assert "not available" in str(
                        e
                    ), f"Error should mention file not available: {e}"
                ...
                result = await tools.act(
                    ...
                    available_file_paths=[
                        test_file_path
                    ],  # File is now in available_file_paths
                    file_system=file_system,
                )
                assert (
                    result.error is None
                ), f"Upload should have succeeded with file in available_file_paths: {result.error}"
                ...
                assert (
                    result.error is None
                ), f"Upload should have succeeded with file in FileSystem: {result.error}"
                ...
                assert (
                    result.error is None
                ), f"Upload should have succeeded with file in downloaded_files: {result.error}"
