...
class UpdateAgentTaskEvent(BaseEvent):
    ...
    user_feedback_type: str | None = Field(
        None, max_length=10
    ...
    def from_agent(cls, agent) -> "UpdateAgentTaskEvent":
        ...
        return cls(
            ...
            device_id=(
                agent.cloud_sync.auth_client.device_id
                if hasattr(agent, "cloud_sync")
                and agent.cloud_sync
                and agent.cloud_sync.auth_client
                else None
            ),
            ...
            agent_state=(
                agent.state.model_dump() if hasattr(agent.state, "model_dump") else {}
            ),
...
class CreateAgentOutputFileEvent(BaseEvent):
    ...
    @classmethod
    async def from_agent_and_file(
        cls, agent, output_path: str
...
class CreateAgentStepEvent(BaseEvent):
    ...
    screenshot_url: str | None = Field(
        None, max_length=MAX_FILE_CONTENT_SIZE
    )  # ~50MB for base64 images
    ...
    def from_agent_step(
        ...
        _first_action = model_output.action[0] if model_output.action else None
        ...
        if browser_state_summary.screenshot:
            ...
            logger.debug(
                f"📸 Including screenshot in CreateAgentStepEvent, length: {len(browser_state_summary.screenshot)}"
            )
        else:
            import logging
        ...
        return cls(
            ...
            evaluation_previous_goal=(
                current_state.evaluation_previous_goal if current_state else ""
            ),
...
class CreateAgentSessionEvent(BaseEvent):
    ...
    def from_agent(cls, agent) -> "CreateAgentSessionEvent":
        ...
        return cls(
            ...
            browser_state={
                "viewport": (
                    agent.browser_profile.viewport
                    if agent.browser_profile
                    else {"width": 1280, "height": 720}
                ),
                "user_agent": (
                    agent.browser_profile.user_agent if agent.browser_profile else None
                ),
                "headless": (
                    agent.browser_profile.headless if agent.browser_profile else True
                ),
            ...
            browser_session_data={
                ...
                "allowed_domains": (
                    agent.browser_profile.allowed_domains
                    if agent.browser_profile
                    else []
                ),
            },
