...
def http_server():
    ...
    server.expect_request(
        "/test", handler_type=HandlerType.PERMANENT
    ).respond_with_data(
...
class TestActionRegistryParameterPatterns:
    ...
    async def test_individual_parameters_no_browser(self, registry):
        ...
        result = await registry.execute_action(
            "simple_action", {"text": "hello", "number": 42}
        )
    ...
    async def test_individual_parameters_with_browser(
        self, registry, browser_session, base_url
    ):
        ...
        event = browser_session.event_bus.dispatch(
            NavigateToUrlEvent(url=f"{base_url}/test", new_tab=True)
        )
        await event

        # Test execution
        result = await registry.execute_action(
            "action_with_browser", {"text": "hello"}, browser_session=browser_session
        )
    ...
    async def test_pydantic_model_parameters(self, registry, browser_session, base_url):
        ...
        async def pydantic_action(
            params: ComplexParams, browser_session: BrowserSession
        ):
            url = await browser_session.get_current_page_url()
            return ActionResult(
    ...
    async def test_mixed_special_parameters(
        self, registry, browser_session, base_url, mock_llm
    ):
...
class TestActionToActionCalling:
    ...
    async def test_action_calling_action_with_kwargs(self, registry, browser_session):
        ...
        async def calling_action(message: str, browser_session: BrowserSession):
            ...
            return ActionResult(
                extracted_content=f"Called result: {intermediate_result.extracted_content}"
            )

        # Test the calling chain
        result = await registry.execute_action(
            "calling_action", {"message": "test"}, browser_session=browser_session
        )

        assert isinstance(result, ActionResult)
        assert result.extracted_content is not None
        assert (
            "Called result: First: Helper processed: test on"
            in result.extracted_content
        )
        assert "/test" in result.extracted_content
    ...
    async def test_google_sheets_style_calling_pattern(self, registry, browser_session):
        ...
        async def _select_cell_or_range(
            browser_session: BrowserSession, cell_or_range: str
        ):
            url = await browser_session.get_current_page_url()
            return ActionResult(
                extracted_content=f"Selected cell {cell_or_range} on {url}"
            )

        @registry.action("Select cell or range")
        async def select_cell_or_range(
            cell_or_range: str, browser_session: BrowserSession
        ):
            # This pattern now works with kwargs
            return await _select_cell_or_range(
                browser_session=browser_session, cell_or_range=cell_or_range
            )

        @registry.action("Select cell or range (fixed)")
        async def select_cell_or_range_fixed(
            cell_or_range: str, browser_session: BrowserSession
        ):
        ...
        async def update_range_contents(
            range_name: str, new_contents: str, browser_session: BrowserSession
        ):
            ...
            _params = action.param_model(cell_or_range=range_name)
            await select_cell_or_range_fixed(
                cell_or_range=range_name, browser_session=browser_session
            )
            return ActionResult(
                extracted_content=f"Updated range {range_name} with {new_contents}"
            )
        ...
        result_chain = await registry.execute_action(
            ...
            {"range_name": "B2:D4", "new_contents": "test data"},
            browser_session=browser_session,
        )
    ...
    async def test_complex_action_chain(self, registry, browser_session):
        ...
        async def middle_action(input_val: str, browser_session: BrowserSession):
            ...
            base_result = await registry.execute_action(
                "base_action",
                {"value": f"processed-{input_val}"},
                browser_session=browser_session,
            )
            return ActionResult(
                extracted_content=f"Middle: {base_result.extracted_content}"
            )
        ...
        async def top_action(original: str, browser_session: BrowserSession):
            ...
            middle_result = await registry.execute_action(
                "middle_action",
                {"input_val": f"enhanced-{original}"},
                browser_session=browser_session,
            )
            return ActionResult(
                extracted_content=f"Top: {middle_result.extracted_content}"
            )

        # Test the full chain
        result = await registry.execute_action(
            "top_action", {"original": "test"}, browser_session=browser_session
        )

        assert isinstance(result, ActionResult)
        assert result.extracted_content is not None
        assert (
            "Top: Middle: Base: processed-enhanced-test on" in result.extracted_content
        )
        assert "/test" in result.extracted_content
...
class TestRegistryEdgeCases:
    ...
    async def test_decorated_action_rejects_positional_args(
        self, registry, browser_session
    ):
        ...
        with pytest.raises(
            TypeError,
            match="test_action\\(\\) does not accept positional arguments, only keyword arguments are allowed",
        ):
            await test_action("A1:B2", browser_session)

        # Test that calling with keyword arguments works
        result = await test_action(
            browser_session=browser_session, cell_or_range="A1:B2"
        )
        assert isinstance(result, ActionResult)
        assert result.extracted_content is not None
    ...
    async def test_missing_required_browser_session(self, registry):
        ...
        with pytest.raises(
            RuntimeError, match="requires browser_session but none provided"
        ):
    ...
    async def test_missing_required_llm(self, registry, browser_session):
        ...
        async def requires_llm(
            ...
            _url = await browser_session.get_current_page_url()
            llm_response = await page_extraction_llm.ainvoke(
                [UserMessage(content="test")]
            )
            return ActionResult(
                extracted_content=f"Text: {text}, LLM: {llm_response.completion}"
            )

        # Should raise RuntimeError when page_extraction_llm is required but not provided
        with pytest.raises(
            RuntimeError, match="requires page_extraction_llm but none provided"
        ):
            await registry.execute_action(
                "requires_llm",
                {"text": "test"},
    ...
    async def test_sync_action_wrapper(self, registry, browser_session):
        ...
        result = await registry.execute_action(
            "sync_action", {"text": "test"}, browser_session=browser_session
        )
    ...
    async def test_excluded_actions(self, browser_session):
        ...
        registry_with_exclusions = Registry[TestContext](
            exclude_actions=["excluded_action"]
        )
        ...
        with pytest.raises(ValueError, match="Action excluded_action not found"):
            await registry_with_exclusions.execute_action(
                "excluded_action", {"text": "test"}
            )

        # Included action should work
        result = await registry_with_exclusions.execute_action(
            "included_action", {"text": "test"}
        )
        assert result.extracted_content is not None
...
class TestExistingToolsActions:
    ...
    async def test_existing_action_models(self, registry, browser_session):
        ...
        async def test_input(params: InputTextAction, browser_session: BrowserSession):
            return ActionResult(
                extracted_content=f"Input text: {params.text} at index: {params.index}"
            )

        # Test SearchGoogleAction
        result1 = await registry.execute_action(
            "test_search", {"query": "python testing"}, browser_session=browser_session
        )
        ...
        result2 = await registry.execute_action(
            "test_click", {"index": 42}, browser_session=browser_session
        )
    ...
    async def test_pydantic_vs_individual_params_consistency(
        self, registry, browser_session
    ):
        ...
        async def individual_params_action(
            text: str, number: int, browser_session: BrowserSession
        ):
        ...
        async def pydantic_params_action(
            params: TestParams, browser_session: BrowserSession
        ):
            return ActionResult(
                extracted_content=f"Pydantic: {params.text}-{params.number}"
            )
        ...
        result1 = await registry.execute_action(
            "individual_params_action", test_data, browser_session=browser_session
        )

        result2 = await registry.execute_action(
            "pydantic_params_action", test_data, browser_session=browser_session
        )
...
class TestValidationRules:
    ...
    def test_error_on_special_param_name_with_wrong_type(self):
        ...
        with pytest.raises(
            ValueError,
            match="conflicts with special argument.*browser_session: BrowserSession",
        ):
...
class TestDecoratedFunctionBehavior:
    ...
    async def test_decorated_function_accepts_params_model(self):
        ...
        result = await input_text(
            params=ParamsModel(index=5, text="hello"),
...
class TestParameterOrdering:
    ...
    def test_extract_content_pattern_registration(self):
        ...
        async def extract_content(
            ...
            return ActionResult(
                extracted_content=f"Goal: {goal}, include_links: {include_links}"
            )
...
class TestParamsModelArgsAndKwargs:
    async def test_browser_session_double_kwarg(self):
        ...
        @registry.action(
            "Google Sheets: Select a cell or range", param_model=CellRangeParams
        )
        async def select_cell_or_range(
            browser_session: BrowserSession, cell_or_range: str
        ):
        ...
        @registry.action(
            "Google Sheets: Select a cell or range (fixed)", param_model=CellRangeParams
        )
        async def select_cell_or_range_fixed(
            browser_session: BrowserSession, cell_or_range: str
        ):
        ...
        @registry.action(
            "Google Sheets: Select with kwargs", param_model=CellRangeParams
        )
        async def select_with_kwargs(
            browser_session: BrowserSession, cell_or_range: str
        ):
        ...
        try:
            _result1 = await registry.execute_action(
                "select_cell_or_range",
                {"cell_or_range": "A1:F100"},
        ...
        try:
            _result2 = await registry.execute_action(
                "select_cell_or_range_fixed",
                {"cell_or_range": "A1:F100"},
                browser_session=browser_session,  # type: ignore
            )
        ...
        try:
            ...
            _parameter_names = ["browser_session", "cell_or_range"]
            ...
            _result3 = await select_cell_or_range(**param_dict, **extra_args)
